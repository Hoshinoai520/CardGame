    ...
    public static class TextureOps
    {
        public static byte[] EncodeTexture(Texture2D texture, string assetPath)
        {
            ...
            switch (extension.ToLowerInvariant())
            {
                ...
                case ".jpg":
                case ".jpeg":
                    return EncodeTextureToJpg(texture);
                default:
        ...
        {
            return EncodeWithImageConversion(texture, "EncodeToPNG");
        }

        public static byte[] EncodeTextureToJpg(Texture2D texture)
        {
            try
            {
                return EncodeWithImageConversion(texture, "EncodeToJPG");
            }
            catch (MissingMethodException)
            {
                return EncodeWithImageConversion(texture, "EncodeToJPG", 75);
            }
        }

        private static byte[] EncodeWithImageConversion(Texture2D texture, string methodName, int? quality = null)
        {
            if (texture == null)
                return null;
            ...
            {
                if (candidate.GetName().Name == "UnityEngine.ImageConversionModule")
                {
                    assembly = candidate;
                    break;
                }
            ...
            assembly ??= System.Reflection.Assembly.Load("UnityEngine.ImageConversionModule");

            var imageConversionType = assembly.GetType("UnityEngine.ImageConversion");
            var parameterTypes = quality.HasValue
            ...
            {
                throw new MissingMethodException("UnityEngine.ImageConversion", methodName);
            }

            object[] args = quality.HasValue
                ? new object[] { texture, quality.Value }
                : new object[] { texture };
            return (byte[])encodeMethod.Invoke(null, args);
        }
