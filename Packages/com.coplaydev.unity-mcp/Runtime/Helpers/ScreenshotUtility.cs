    ...
    public static class ScreenshotUtility
    {
            ...
            try
            {
                ...
                if (includeImage)
                {
                    ...
                    if (width > targetMax || height > targetMax)
                    {
                        ...
                        byte[] smallPng = EncodeTextureToPng(downscaled);
        ...
        public static (string base64, int width, int height) RenderCameraToBase64(Camera camera, int maxResolution = 640)
        {
            ...
            try
            {
                ...
                {
                    downscaled = DownscaleTexture(tex, targetMax);
                    string b64 = System.Convert.ToBase64String(EncodeTextureToPng(downscaled));
                    return (b64, downscaled.width, downscaled.height);
                }
                else
                {
                    string b64 = System.Convert.ToBase64String(EncodeTextureToPng(tex));
            ...
            List<Texture2D> tiles, List<string> labels, int padding = 4)
        {
            ...
            try
            {
                ...
                byte[] png = EncodeTextureToPng(sheet);
        ...
        private static byte[] EncodeTextureToPng(Texture2D texture)
        {
            ...
            {
                throw new InvalidOperationException("UnityEngine.ImageConversion.EncodeToPNG is unavailable.");
            }
