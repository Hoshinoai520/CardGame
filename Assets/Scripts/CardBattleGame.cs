...
public sealed class CardBattleGame : MonoBehaviour
{
    ...
    {
        if (UseReferenceSketchLayout())
        {
            BuildReferenceSketchLayout();
            return;
        }

        var playerEffects = Zone("我方效果", 18, 22, 264, 174);
        debuffText = Label(playerEffects, "暂无负面状态", 58, -52, 176, 52, 14, TextAnchor.UpperLeft, ColorText());
        Label(playerEffects, "负面状态", 58, -34, 98, 18, 11, TextAnchor.MiddleLeft, Blue());
        var debuffIcon = CreatePanel(playerEffects, "Debuff Icon", 18, -54, 28, 28, Blue());
        Label(debuffIcon, "!", 0, 0, 28, 28, 18, TextAnchor.MiddleCenter, new Color32(10, 18, 22, 255));
        var focusIcon = CreatePanel(playerEffects, "Focus Icon", 18, -116, 28, 28, new Color32(90, 205, 118, 255));
        Label(focusIcon, "+", 0, 0, 28, 28, 18, TextAnchor.MiddleCenter, new Color32(8, 18, 10, 255));
        Label(playerEffects, "专注\n抽牌 +1", 58, -110, 120, 42, 14, TextAnchor.UpperLeft, ColorText());
        Label(playerEffects, "1 回合", 196, -122, 48, 18, 12, TextAnchor.MiddleRight, Muted());

        var actionPanel = Zone("我方行动", 18, 232, 264, 220);
        for (var i = 0; i < 4; i++)
        {
            var item = CreatePanel(actionPanel, "Action Row", 18, -72 - i * 36, 228, 30, new Color32(16, 25, 28, 190));
            AddOutline(item.gameObject, new Color32(220, 190, 130, 60), new Vector2(1, -1));
            item.SetAsFirstSibling();
        }
        turnText = Label(actionPanel, "我方行动", 26, -42, 210, 26, 18, TextAnchor.MiddleCenter, ColorText());
        Label(actionPanel, "攻击", 62, -72, 160, 30, 15, TextAnchor.MiddleLeft, ColorText());
        Label(actionPanel, "技能", 62, -108, 160, 30, 15, TextAnchor.MiddleLeft, ColorText());
        Label(actionPanel, "防御", 62, -144, 160, 30, 15, TextAnchor.MiddleLeft, ColorText());
        Label(actionPanel, "道具", 62, -180, 160, 30, 15, TextAnchor.MiddleLeft, ColorText());

        var leftDiscard = Zone("弃牌", 18, 486, 130, 184);
        DrawDeckStack(leftDiscard);
        Label(leftDiscard, "12", 0, -154, 130, 24, 20, TextAnchor.MiddleCenter, Gold());
        var leftDraw = Zone("抽牌", 160, 486, 130, 184);
        DrawDeckStack(leftDraw);
        Label(leftDraw, "18", 0, -154, 130, 24, 20, TextAnchor.MiddleCenter, Gold());

        var playerPanel = Zone("我方", 18, 696, 344, 176);
        DrawHeroArt(playerPanel, 18, -42, 118, 116);
        playerHpFill = CreateStatBar(playerPanel, 176, -60, 136, 12, new Color32(236, 72, 60, 255));
        CreateStatBar(playerPanel, 176, -96, 136, 12, Blue());
        CreateStatBar(playerPanel, 176, -132, 136, 12, Gold());
        playerHpText = Label(playerPanel, "生命 50", 170, -40, 132, 20, 13, TextAnchor.MiddleLeft, ColorText());
        playerBlockText = Label(playerPanel, "护甲 0", 170, -76, 132, 20, 13, TextAnchor.MiddleLeft, ColorText());
        energyText = Label(playerPanel, "能量 3", 170, -112, 132, 20, 13, TextAnchor.MiddleLeft, ColorText());
        playerCardHpText = playerHpText;
        playerCardBlockText = playerBlockText;

        enemyActiveCard = Zone(string.Empty, 677, 26, 170, 220);
        ApplyTexture(enemyActiveCard, new Color32(60, 25, 28, 255), new Color32(116, 48, 44, 255), Gold());
        DrawEnemyArt(enemyActiveCard, 18, -34, 134, 118);
        Label(enemyActiveCard, "暗影骑士", 24, -154, 122, 22, 13, TextAnchor.MiddleCenter, ColorText());
        Label(enemyActiveCard, "6", 8, -10, 34, 34, 21, TextAnchor.MiddleCenter, ColorText());
        Label(enemyActiveCard, "5", 12, -184, 34, 26, 20, TextAnchor.MiddleCenter, Gold());
        Label(enemyActiveCard, "7", 124, -184, 34, 26, 20, TextAnchor.MiddleCenter, Red());

        enemyAttackCard = Zone("敌方行动 / 意图", 404, 28, 270, 150);
        var intentBox = CreatePanel(enemyAttackCard, "Intent Icon", 28, -52, 40, 40, Red());
        intentIconText = Label(intentBox, "攻", 0, 0, 40, 40, 20, TextAnchor.MiddleCenter, new Color32(32, 20, 10, 255));
        intentNameText = Label(enemyAttackCard, "强力打击", 82, -46, 154, 24, 16, TextAnchor.MiddleLeft, ColorText());
        intentDescText = Label(enemyAttackCard, "造成 8 点伤害。", 82, -72, 166, 36, 13, TextAnchor.UpperLeft, Muted());
        enemyActionText = Label(enemyAttackCard, "等待我方结束回合", 28, -116, 214, 22, 13, TextAnchor.MiddleCenter, ColorText());

        var enemyEffects = Zone("敌方效果", 1026, 24, 260, 172);
        CreatePanel(enemyEffects, "Rage Icon", 18, -54, 28, 28, Red());
        CreatePanel(enemyEffects, "Fortify Icon", 18, -114, 28, 28, Blue());
        Label(enemyEffects, "狂怒\n攻击 +2", 58, -52, 112, 42, 14, TextAnchor.UpperLeft, ColorText());
        Label(enemyEffects, "2 回合", 184, -62, 58, 18, 12, TextAnchor.MiddleRight, Muted());
        Label(enemyEffects, "固守\n护甲 +3", 58, -112, 112, 42, 14, TextAnchor.UpperLeft, ColorText());
        Label(enemyEffects, "1 回合", 184, -122, 58, 18, 12, TextAnchor.MiddleRight, Muted());

        var enemyPanel = Zone("敌方", 1312, 22, 170, 238);
        DrawEnemyArt(enemyPanel, 14, -42, 142, 90);
        enemyHpFill = CreateStatBar(enemyPanel, 44, -156, 104, 12, new Color32(236, 72, 60, 255));
        CreateStatBar(enemyPanel, 44, -192, 104, 12, Blue());
        enemyHpText = Label(enemyPanel, "生命 60/60", 44, -132, 104, 20, 13, TextAnchor.MiddleLeft, ColorText());
        enemyBlockText = Label(enemyPanel, "护甲 0", 44, -168, 104, 20, 13, TextAnchor.MiddleLeft, ColorText());

        var enemyStatus = Zone("状态", 1312, 274, 170, 162);
        Label(enemyStatus, "流血", 22, -48, 82, 22, 13, TextAnchor.MiddleLeft, ColorText());
        Label(enemyStatus, "2", 124, -48, 24, 22, 13, TextAnchor.MiddleRight, ColorText());
        Label(enemyStatus, "虚弱", 22, -84, 82, 22, 13, TextAnchor.MiddleLeft, ColorText());
        Label(enemyStatus, "1", 124, -84, 24, 22, 13, TextAnchor.MiddleRight, ColorText());
        Label(enemyStatus, "易伤", 22, -120, 82, 22, 13, TextAnchor.MiddleLeft, ColorText());
        Label(enemyStatus, "1", 124, -120, 24, 22, 13, TextAnchor.MiddleRight, ColorText());

        var rightDiscard = Zone("弃牌", 1314, 464, 78, 174);
        DrawDeckStack(rightDiscard);
        Label(rightDiscard, "09", 0, -142, 78, 24, 18, TextAnchor.MiddleCenter, Gold());
        var rightDraw = Zone("抽牌", 1406, 464, 78, 174);
        DrawDeckStack(rightDraw);
        Label(rightDraw, "14", 0, -142, 78, 24, 18, TextAnchor.MiddleCenter, Gold());

        BuildBattleSlots();
        BuildPlayerCharacter();
        BuildHand();

        var playerStatus = Zone("状态", 378, 810, 300, 64);
        Label(playerStatus, "力量  攻击 +2", 48, -34, 112, 20, 12, TextAnchor.MiddleLeft, ColorText());
        Label(playerStatus, "敏捷  抽牌 +1", 182, -34, 104, 20, 12, TextAnchor.MiddleLeft, ColorText());

        var energyPanel = Zone("能量", 696, 810, 260, 64);
        statusBar = Label(energyPanel, "第 1 回合 / 我方行动", 16, -36, 228, 20, 13, TextAnchor.MiddleCenter, Muted());

        endTurnButton = Button(table, "结束回合", 978, -820, 170, 56, () => EndTurn());
        Button(table, "重开", 1162, -828, 76, 40, () => ResetGame(), true);
    }

    private bool UseReferenceSketchLayout()
    {
        return true;
    }
